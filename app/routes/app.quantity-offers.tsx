                        ...
                        <div className="preview-panel">
                            ...
                            <div className="preview-content" style={previewDevice === 'desktop' ? { display: 'flex', justifyContent: 'center', alignItems: 'flex-start', background: '#f3f4f6', padding: '32px 16px', borderRadius: '12px', minHeight: '550px' } : {}}>
                                <div className={previewDevice === 'mobile' ? "preview-phone" : "preview-desktop"} style={previewDevice === 'desktop' ? { width: '100%', maxWidth: '800px', margin: '0 auto', position: 'relative' } : {}}>
                                    <div className="preview-phone-screen" onScroll={handlePreviewAutoHideScroll} style={previewDevice === 'desktop' ? { background: '#ffffff', borderRadius: '12px', height: 'auto', maxHeight: 'none', padding: '24px', maskImage: 'none', WebkitMaskImage: 'none', overflow: 'visible', boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.1), 0 2px 4px -1px rgba(0, 0, 0, 0.06)' } : {
                                                        ...
                                                        .map((field: any) => {
                                                            ...
                                                            if (field.id === 'payment_mode') {
                                                                ...
                                                                if (field.id === 'payment_mode') {
                                                                    ...
                                                                    const discountPercent = selectedOffer?.discountPercent || 0;
                                                                    const subtotal = samplePrice * qtyForPayment;
                                                                    const discount = subtotal * (discountPercent / 100);
                                                                    const shippingEnabled = formSettings?.shipping_options?.enabled;
                                                                    const shippingOption = formSettings?.shipping_options?.options?.find((o: any) => o.id === formSettings?.shipping_options?.defaultOption);
                                                                    const shippingCost = shippingEnabled ? (shippingOption?.price || 0) : 0;
                                                                                    ...
                                                                                    <label style={{
                                                                                        ...
                                                                                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: (partialPaymentSettings?.payment_method_tags?.full_prepaid?.enabled || (!partialPaymentSettings?.payment_method_tags?.full_prepaid && true)) ? '18px 10px 8px 10px' : '8px 10px' }}>
                                                                                            ...
                                                                                            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexShrink: 0 }}>
                                                                                                ...
                                                                                                <input type="radio" name="qo-preview-payment" checked readOnly style={{ width: '18px', height: '18px', accentColor: '#22c55e', margin: 0, cursor: 'pointer' }} />
                                                                                            </div>
                                                                                        </div>
                                                                                        {/* Info Bar */}
                                                                                    ...
                                                                                    <label style={{
                                                                                        ...
                                                                                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: partialPaymentSettings?.payment_method_tags?.partial_payment?.enabled ? '18px 10px 8px 10px' : '8px 10px' }}>
                                                                                            ...
                                                                                            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexShrink: 0 }}>
                                                                                                ...
                                                                                                <input type="radio" name="qo-preview-payment" disabled style={{ width: '18px', height: '18px', margin: 0 }} />
                                                                                            </div>
                                                                                        </div>
                                                                                        {/* Info Bar */}
                                                            ...
                                                            // Skip quantity field (handled separately)
                                                            ...
                                                            const fieldIcons: Record<string, React.ReactNode> = {
                                                                ...
                                                                star: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke={iconColor} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2" /></svg>,
                                                                note: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke={iconColor} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z" /><polyline points="14,2 14,8 20,8" /><line x1="16" y1="13" x2="8" y2="13" /><line x1="16" y1="17" x2="8" y2="17" /><polyline points="10,9 9,9 8,9" /></svg>,
                                                                location: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke={iconColor} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z" /><circle cx="12" cy="10" r="3" /></svg>,
                                                            };
                                                            ...
                                                            const isCityVisible = (mergedFields as any[]).some((f: any) => f.id === 'city' && f.visible);
                                                            const isZipVisible = (mergedFields as any[]).some((f: any) => (f.id === 'zip' || f.id === 'zipcode') && f.visible);
                                                                ...
                                                                <div key={field.id} style={{
                                                                    ...
                                                                    <div style={{ position: 'relative' }}>
                                                                        ...
                                                                        {field.id === 'state' && (formSettings?.blocks?.enable_state_province ?? true) && (
                                                                            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" width="16" height="16" style={{ position: 'absolute', right: '12px', top: '13px', fill: 'none', stroke: textColor, strokeWidth: 2, strokeLinecap: 'round', strokeLinejoin: 'round', pointerEvents: 'none', opacity: 0.7 }}>
