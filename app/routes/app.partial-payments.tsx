        ...
        <BlockStack gap="500">
          ...
          <Card>
            <BlockStack gap="400">
              ...
              {settings.payment_options.map((opt, idx) => {
                let displayValue = '';
                  ...
                  <div key={opt.id} className="pp-option-box">
                    <div className="pp-option-box-header">
                      <div className="pp-option-box-title">
                        <span className="pp-option-number-badge">{idx + 1}</span>
          ...
          {/* ════════════════════════════════════════════════
              CARD 4 — APPEARANCE & MODAL CUSTOMIZATION
          ════════════════════════════════════════════════ */}
          {/* ════════════════════════════════════════════════
              CARD 4 — APPEARANCE & MODAL CUSTOMIZATION (SETTINGS ONLY)
          ════════════════════════════════════════════════ */}
          <Card>
            <BlockStack gap="400">
              <BlockStack gap="100">
                <Text as="h2" variant="headingMd">Appearance & Modal Customization</Text>
                <Text as="p" variant="bodySm" tone="subdued">
                  Customize the look and copy of the payment selection modal shown to customers.
                </Text>
              </BlockStack>
          ...
          {/* ════════════════════════════════════════════════
              CARD 5 — MODAL PREVIEW (SEPARATE CARD)
          ════════════════════════════════════════════════ */}
          <Card>
            <BlockStack gap="400">
              <div className="pp-preview-card-header">
                <Text as="h2" variant="headingMd">Modal Preview</Text>
                <button
                  type="button"
                  className="pp-preview-toggle-btn"
                  onClick={() => setShowPreview(!showPreview)}
                >
                  {showPreview ? (
                    <>
                      Hide Preview <span style={{ fontSize: '10px' }}>▲</span>
                    </>
                  ) : (
                    <>
                      Show Preview <span style={{ fontSize: '10px' }}>▼</span>
                    </>
                  )}
                </button>
              </div>
              ...
              {showPreview && (
                <div className="pp-preview-viewport">
                  <div className="pp-preview-modal-box">
                    
                    {/* Left Panel */}
                    <div
                      className="pp-preview-left-panel"
                      style={{
                        background: `linear-gradient(160deg, ${ms.left_gradient_start || ms.left_bg_color} 0%, ${ms.left_gradient_end || ms.left_bg_color} 100%)`,
                        color: ms.left_text_color,
                      }}
                    >
                      <div className="pp-preview-shop-pill">
                        <svg viewBox="0 0 20 20" fill="currentColor" style={{ width: '12px', height: '12px' }}>
                          <path d="M3 5a2 2 0 012-2h10a2 2 0 012 2v8a2 2 0 01-2 2h-2.22l-.11.22-1.5 3a1 1 0 01-1.78 0l-1.5-3-.11-.22H5a2 2 0 01-2-2V5zm2 2v6h10V7H5zm8 4H7V9h6v2z" />
                        </svg>
                        {shopDomain || 'fox-cod.myshopify.com'}
                      </div>

                      <div className="pp-preview-order-summary-box">
                        <div className="pp-preview-summary-header">
                          <span>Order summary</span>
                          <span>❯</span>
                        </div>
                        <div className="pp-preview-product-row">
                          <div className="pp-preview-product-img">
                            <div style={{ width: '20px', height: '40px', background: '#2dd4bf', borderRadius: '4px' }} />
                          </div>
                          <div className="pp-preview-product-info">
                            <span className="pp-preview-product-name">The Complete Snowboard</span>
                            <span className="pp-preview-product-sub">Full price</span>
                            <span className="pp-preview-product-price">$699.95</span>
                            <span className="pp-preview-product-qty">Quantity: 1</span>
                          </div>
                        </div>
                      </div>
                    </div>
                    ...
                    {/* Right Panel */}
                    <div
                      className="pp-preview-right-panel"
                      style={{
                        background: ms.right_bg_color,
                      }}
                    >
                      <button type="button" className="pp-preview-close-btn" aria-label="Close">
                        <svg viewBox="0 0 20 20" fill="currentColor" style={{ width: '16px', height: '16px' }}>
                          <path fillRule="evenodd" d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z" clipRule="evenodd" />
                        </svg>
                      </button>

                      <h3 className="pp-preview-right-title">
                        {ms.title || 'Choose Your Payment Option'}
                      </h3>

                      <div className="pp-preview-options-list">
                        {settings.payment_options.length === 0 ? (
                          <div className="pp-preview-opt-card" style={{ borderColor: ms.right_border_color, color: '#9ca3af', justifyContent: 'center' }}>
                            Add payment options in settings
                          </div>
                        ) : (
                          settings.payment_options.map((opt, i) => {
                            const isFirst = i === 0;
                            // Calculate display value (based on $699.95)
                            let optVal = '';
                            let optSub = '';
                            const dummyTotal = 699.95;
                            if (opt.type === 'percentage' || opt.type === 'remaining_percentage') {
                              const deposit = (dummyTotal * opt.value) / 100;
                              const remaining = dummyTotal - deposit;
                              optVal = `$${deposit.toFixed(2)}`;
                              optSub = `Total: $${dummyTotal.toFixed(2)}. Pay now ${opt.value}% ($${deposit.toFixed(2)}), $${remaining.toFixed(2)} remaining on delivery`;
                            } else {
                              const deposit = opt.value;
                              const remaining = dummyTotal - deposit;
                              optVal = `$${deposit.toFixed(2)}`;
                              optSub = `Total: $${dummyTotal.toFixed(2)}. Pay now $${deposit.toFixed(2)}, $${remaining.toFixed(2)} remaining on delivery`;
                            }

                            return (
                              <div
                                key={opt.id}
                                className={`pp-preview-opt-card ${isFirst ? 'selected' : ''}`}
                                style={{
                                  borderColor: isFirst ? ms.button_color : ms.right_border_color,
                                  background: isFirst ? `${ms.button_color}06` : '#ffffff',
                                }}
                              >
                                <div className="pp-preview-opt-left">
                                  <div
                                    className="pp-preview-opt-icon-wrap"
                                    style={{
                                      color: ms.button_color,
                                    }}
                                  >
                                  ...
                                  <div className="pp-preview-opt-text">
                                    <span className="pp-preview-opt-label">{opt.label}</span>
                                    <span className="pp-preview-opt-sub">{optSub}</span>
                                  </div>
                                </div>
                                <div className="pp-preview-opt-right">
                                  <span className="pp-preview-opt-price">{optVal}</span>
                                  <span className="pp-preview-opt-taxes">Taxes computed later</span>
                                  {isFirst && (
                                    <div
                                      className="pp-preview-opt-tick"
                                      style={{
                                        background: ms.button_color,
                                        color: ms.button_text_color,
                                      }}
                                    >
                                      <svg viewBox="0 0 20 20" fill="currentColor" style={{ width: '12px', height: '12px' }}>
                      ...
                      <div>
                        <button
                          type="button"
                          className="pp-preview-checkout-btn"
                          style={{
                            background: ms.button_color,
                            color: ms.button_text_color,
                          }}
                        ...
                        {ms.show_security_icons && (
                          <div className="pp-preview-trust-badges">
                            <div className="pp-preview-trust-item">
                              <span className="pp-preview-trust-icon">
                                <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" style={{ width: '20px', height: '20px' }}>
                        ...
                        <hr className="pp-preview-divider" />
