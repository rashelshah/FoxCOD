...
const PreviewDisplay = memo(({
    ...
    const FieldIconSvg = ({ field, isTextarea }: { field: FormField; isTextarea?: boolean }) => {
        ...
        const svgs: Record<string, React.ReactNode> = {
            ...
            note: <svg style={s} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z" /><polyline points="14,2 14,8 20,8" /><line x1="16" y1="13" x2="8" y2="13" /><line x1="16" y1="17" x2="8" y2="17" /><polyline points="10,9 9,9 8,9" /></svg>,
            location: <svg style={s} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z" /><circle cx="12" cy="10" r="3" /></svg>
        };
        if (field.isCustom && field.iconType && svgs[field.iconType]) {
            return svgs[field.iconType];
        }
        return svgs[field.id] || svgs.text;
    };
...
export default function SettingsPage() {
    const { shop, settings, shippingRates: initialShippingRates, products, collections, shopCurrency, partialPaymentSettings } = useLoaderData<typeof loader>();
    ...
    const DropdownIcon = ({ iconType }: { iconType: string }) => {
        ...
        if (iconType === 'star') return <svg style={s} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2" /></svg>;
        if (iconType === 'note') return <svg style={s} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z" /><polyline points="14,2 14,8 20,8" /><line x1="16" y1="13" x2="8" y2="13" /><line x1="16" y1="17" x2="8" y2="17" /><polyline points="10,9 9,9 8,9" /></svg>;
        if (iconType === 'location') return <svg style={s} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z" /><circle cx="12" cy="10" r="3" /></svg>;
        return <svg style={s} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="4 7 4 4 20 4 20 7" /><line x1="9" y1="20" x2="15" y2="20" /><line x1="12" y1="4" x2="12" y2="20" /></svg>;
    };
    ...
    const addCustomField = useCallback(() => {
        ...
        setNewFieldOptions('');
        setNewFieldIconType('text');
        setShowAddFieldModal(false);
    }, [newFieldLabel, newFieldType, newFieldPlaceholder, newFieldOptions, newFieldIconType, fields]);
                                                            ...
                                                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
                                                                <div style={{ display: 'flex', gap: '4px' }}>
                                                                    <button
                                                                        ...
                                                                        style={{ padding: '6px 10px', minWidth: 'auto', background: '#fff' }}
                                                                ...
                                                                <button
                                                                    type="button"
                                                                    style={{ display: 'flex', alignItems: 'center', gap: '4px', background: 'none', border: 'none', color: '#991b1b', fontSize: '13px', fontWeight: 500, cursor: 'pointer' }}
                                                                    onClick={() => { setButtonStylesState(s => ({ ...s, iconType: 'none' })); setBtnIconPopoverActive(false); }}
                                                                >
                                                                    <svg style={{ width: 14, height: 14 }} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="3 6 5 6 21 6"></polyline><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"></path></svg>
                                                                    Remove
                                                                </button>
                                                            </div>
                                                            <div style={{ height: '1px', background: '#e5e7eb', margin: '0 -12px 12px -12px' }}></div>
                                                            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
                                                                {['cart', 'cart-plus', 'cart-check', 'bag', 'bag-check', 'basket', 'box', 'card', 'cash', 'wallet', 'checkout', 'truck', 'whatsapp', 'arrow-right', 'lock'].map((iconName) => (
                                                                    <button
                                                                        type="button"
                                                                        key={iconName}
                                                                        style={{
                                                                            ...
                                                                            border: buttonStylesState?.iconType === iconName ? '2px solid #5c6ac4' : '1px solid #e5e7eb',
                                                                            borderRadius: '8px',
                                                                            background: buttonStylesState?.iconType === iconName ? '#f6f6f7' : '#fff',
                                                                            color: buttonStylesState?.iconType === iconName ? '#111827' : '#333',
                                                                            cursor: 'pointer'
                                                                        }}
                                                                        onClick={() => { setButtonStylesState(s => ({ ...s, iconType: iconName })); setBtnIconPopoverActive(false); }}
                                                                    >
                                                                        <ButtonIconSvg iconType={iconName} color={buttonStylesState?.iconType === iconName ? "#111827" : "#333"} />
                                                                    </button>
                                                                ))}
                                                            </div>
                                                        </div>
                            ...
                            {activeTab === 'form' && (
                                <>
                                    ...
                                    <AccordionSection id="field-styling" tab="form" title="Field Styling" helperText="Changes the styling of the form fields" expandedSection={expandedSection} toggleSection={toggleSection}>
                                        ...
                                        {(() => {
                                            ...
                                            const presetMap: Record<string, any> = {
                                                ...
                                                arctic_blue: { styles: { themeKey: 'arctic_blue', textColor: '#164e63', textSize: 14, fontStyle: 'normal' as const, borderColor: '#67e8f9', borderWidth: 1, background: '#ecfeff', backgroundColor: '#ecfeff', labelAlignment: 'left' as const, iconColor: '#0891b2', iconBackground: '#cffafe', borderRadius: 12, shadow: true, fieldBackgroundColor: '#ecfeff', labelColor: '#155e75', labelFontSize: 14, priceColor: '#0891b2' }, buttonColor: '#0891b2' },
                                                rose_garden: { styles: { themeKey: 'rose_garden', textColor: '#9f1239', textSize: 14, fontStyle: 'normal' as const, borderColor: '#fda4af', borderWidth: 1, background: '#fff1f2', backgroundColor: '#fff1f2', labelAlignment: 'left' as const, iconColor: '#e11d48', iconBackground: '#ffe4e6', borderRadius: 14, shadow: true, fieldBackgroundColor: '#fff1f2', labelColor: '#881337', labelFontSize: 14, priceColor: '#e11d48' }, buttonColor: '#e11d48' },
                                                forest_green: { styles: { themeKey: 'forest_green', textColor: '#14532d', textSize: 14, fontStyle: 'normal' as const, borderColor: '#86efac', borderWidth: 1, background: '#f0fdf4', backgroundColor: '#f0fdf4', labelAlignment: 'left' as const, iconColor: '#16a34a', iconBackground: '#dcfce7', borderRadius: 10, shadow: true, fieldBackgroundColor: '#f0fdf4', labelColor: '#15803d', labelFontSize: 14, priceColor: '#16a34a' }, buttonColor: '#16a34a' },
                                                professional: { styles: { themeKey: 'professional', textColor: '#1f2937', textSize: 14, fontStyle: 'normal' as const, borderColor: '#9ca3af', borderWidth: 1, background: '#f9fafb', backgroundColor: '#f9fafb', labelAlignment: 'left' as const, iconColor: '#4b5563', iconBackground: '#e5e7eb', borderRadius: 6, shadow: false, fieldBackgroundColor: '#ffffff', labelColor: '#111827', labelFontSize: 13, priceColor: '#374151' }, buttonColor: '#374151' },
                                                minimal_white: { styles: { themeKey: 'minimal_white', textColor: '#374151', textSize: 14, fontStyle: 'normal' as const, borderColor: '#e5e7eb', borderWidth: 1, background: '#ffffff', backgroundColor: '#ffffff', labelAlignment: 'left' as const, iconColor: '#9ca3af', iconBackground: 'transparent', borderRadius: 8, shadow: false, fieldBackgroundColor: '#ffffff', labelColor: '#6b7280', labelFontSize: 13, priceColor: '#111827' }, buttonColor: '#111827' },
                                                luxury_gold: { styles: { themeKey: 'luxury_gold', textColor: '#fbbf24', textSize: 14, fontStyle: 'normal' as const, borderColor: '#d97706', borderWidth: 1, background: '#18181b', backgroundColor: '#18181b', labelAlignment: 'left' as const, iconColor: '#f59e0b', iconBackground: '#27272a', borderRadius: 10, shadow: true, fieldBackgroundColor: '#27272a', labelColor: '#fcd34d', labelFontSize: 14, priceColor: '#fbbf24' }, buttonColor: '#d97706' },
                                                ocean_breeze: { styles: { themeKey: 'ocean_breeze', textColor: '#0f766e', textSize: 14, fontStyle: 'normal' as const, borderColor: '#5eead4', borderWidth: 1, background: '#f0fdfa', backgroundColor: '#f0fdfa', labelAlignment: 'left' as const, iconColor: '#14b8a6', iconBackground: '#ccfbf1', borderRadius: 14, shadow: true, fieldBackgroundColor: '#f0fdfa', labelColor: '#115e59', labelFontSize: 14, priceColor: '#14b8a6' }, buttonColor: '#14b8a6' },
                                                royal_indigo: { styles: { themeKey: 'royal_indigo', textColor: '#e0e7ff', textSize: 14, fontStyle: 'normal' as const, borderColor: '#6366f1', borderWidth: 1, background: '#1e1b4b', backgroundColor: '#1e1b4b', labelAlignment: 'left' as const, iconColor: '#a5b4fc', iconBackground: '#312e81', borderRadius: 12, shadow: true, fieldBackgroundColor: '#1e1b4b', labelColor: '#f8fafc', labelFontSize: 14, priceColor: '#a5b4fc' }, buttonColor: '#6366f1' },
                                                emerald_night: { styles: { themeKey: 'emerald_night', textColor: '#ecfdf5', textSize: 14, fontStyle: 'normal' as const, borderColor: '#10b981', borderWidth: 1, background: '#022c22', backgroundColor: '#022c22', labelAlignment: 'left' as const, iconColor: '#6ee7b7', iconBackground: '#064e3b', borderRadius: 12, shadow: true, fieldBackgroundColor: '#022c22', labelColor: '#f0fdf4', labelFontSize: 14, priceColor: '#34d399' }, buttonColor: '#10b981' },
                                            };

                                            // Active category is stored in formStyles._cat (written on tab click)
                                            ...
                                            const visiblePresets = activeCategory === 'all'
                                                ? presetList
                                                : presetList.filter(p => p.category === activeCategory || p.key === 'custom');
                                                            ...
                                                            {categories.map(cat => (
                                                                <button
                                                                    ...
                                                                    style={{
                                                                        padding: '5px 13px',
                                                                        borderRadius: 20,
                                                                        border: `1.5px solid ${activeCategory === cat.key ? '#111827' : '#e5e7eb'}`,
                                                                        background: activeCategory === cat.key ? '#111827' : '#ffffff',
                                                                    ...
                                                                    onClick={() => {
                                                                        setFormStyles((s: any) => ({ ...s, _cat: cat.key }));
                                                                    }}
                                                                >
                                                                    {cat.label}
                                                                </button>
                                                                    ...
                                                                    <div
                                                                        ...
                                                                        onClick={() => {
                                                                            ...
                                                                            if (key !== 'custom' && presetMap[key]) {
                                                                                ...
                                                                                setFormStyles((s: any) => ({ ...s, themeKey: 'custom' }));
                                                                            }
                                                                        }}
                                                                        title={preset.name}
                                                                        style={{
                                                                    ...
                                                                    >
                                                                        ...
                                                                        <div style={{
                                                                            ...
                                                                            <div style={{
                                                                                ...
                                                                                borderRadius: 5,
                                                                                flexShrink: 0,
                                                                                background: preset.fieldBg === preset.bg
                                    ...
                                    <Modal
                                        ...
                                        onClose={() => { setShowAddFieldModal(false); setNewFieldLabel(''); setNewFieldPlaceholder(''); setNewFieldOptions(''); setNewFieldIconType('text'); }}
                                        ...
                                        secondaryActions={[{
                                            content: 'Cancel',
                                            onAction: () => { setShowAddFieldModal(false); setNewFieldLabel(''); setNewFieldPlaceholder(''); setNewFieldOptions(''); setNewFieldIconType('text'); },
                                        }]}
                                    >
                                        <Modal.Section>
                                            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                                                ...
                                                <Popover
                                                    ...
                                                    activator={
                                                        <div style={{ marginTop: '4px' }}>
                                                            <div style={{ marginBottom: '4px', fontSize: '13px', color: 'rgb(48, 48, 48)' }}>Field Icon</div>
                                                            <Button
                                                                fullWidth
                                                                textAlign="left"
                                                                disclosure
                                                                onClick={() => setIconPopoverActive(!iconPopoverActive)}
                                                                icon={
                                                                    <span style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: '20px', height: '20px', color: '#333' }}>
                                                                        <DropdownIcon iconType={newFieldIconType} />
                                                                    </span>
                                                                }
                                                            >
                                                                {
                                                                    [
                                                                        { label: 'Text/General', value: 'text' },
                                                                        { label: 'User/Person', value: 'user' },
                                                                        { label: 'Hash (#) / Number', value: 'hash' },
                                                                        { label: 'Phone', value: 'phone' },
                                                                        { label: 'Email', value: 'email' },
                                                                        { label: 'List/Dropdown', value: 'list' },
                                                                        { label: 'Note/Document', value: 'note' },
                                                                        { label: 'Location/Pin', value: 'location' },
                                                                        { label: 'Star', value: 'star' }
                                                                    ].find(opt => opt.value === newFieldIconType)?.label || 'Text/General'
                                                                }
                                                            </Button>
                                                            <div style={{ marginTop: '4px', fontSize: '12px', color: 'rgb(109, 113, 117)' }}>Select an icon to display next to your custom field.</div>
                                                        </div>
                                                    }
                                                ...
                                                >
                                                    <ActionList
                                                        actionRole="menuitem"
                                                        items={[
                                                            { content: 'Text/General', icon: () => <span style={{ width: 20, display: 'flex' }}><DropdownIcon iconType="text" /></span>, onAction: () => { setNewFieldIconType('text'); setIconPopoverActive(false); } },
                                                            { content: 'User/Person', icon: () => <span style={{ width: 20, display: 'flex' }}><DropdownIcon iconType="user" /></span>, onAction: () => { setNewFieldIconType('user'); setIconPopoverActive(false); } },
                                                            { content: 'Hash (#) / Number', icon: () => <span style={{ width: 20, display: 'flex' }}><DropdownIcon iconType="hash" /></span>, onAction: () => { setNewFieldIconType('hash'); setIconPopoverActive(false); } },
                                                            { content: 'Phone', icon: () => <span style={{ width: 20, display: 'flex' }}><DropdownIcon iconType="phone" /></span>, onAction: () => { setNewFieldIconType('phone'); setIconPopoverActive(false); } },
                                                            { content: 'Email', icon: () => <span style={{ width: 20, display: 'flex' }}><DropdownIcon iconType="email" /></span>, onAction: () => { setNewFieldIconType('email'); setIconPopoverActive(false); } },
                                                            { content: 'List/Dropdown', icon: () => <span style={{ width: 20, display: 'flex' }}><DropdownIcon iconType="list" /></span>, onAction: () => { setNewFieldIconType('list'); setIconPopoverActive(false); } },
                                                            { content: 'Note/Document', icon: () => <span style={{ width: 20, display: 'flex' }}><DropdownIcon iconType="note" /></span>, onAction: () => { setNewFieldIconType('note'); setIconPopoverActive(false); } },
                                                            { content: 'Location/Pin', icon: () => <span style={{ width: 20, display: 'flex' }}><DropdownIcon iconType="location" /></span>, onAction: () => { setNewFieldIconType('location'); setIconPopoverActive(false); } },
                                                            { content: 'Star', icon: () => <span style={{ width: 20, display: 'flex' }}><DropdownIcon iconType="star" /></span>, onAction: () => { setNewFieldIconType('star'); setIconPopoverActive(false); } }
                                                        ]}
                                                    />
                                                </Popover>
